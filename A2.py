...
def main():
    ...
    def generate_candidates(df, is_train=True):
        ...
        for idx, row in df.iterrows():
            ...
            if not seq:
                ...
                candidates = []
                # 核心召回1：最近6个互动商品的相似交叉商品，扩大召回范围
                for ri in seq[-6:]:
                    sim_items = [k for k, v in
                                 sorted(item_sim_matrix[ri].items(), key=lambda x: x[1], reverse=True)[:30]]
    ...
    # ==================== 新增：合并用户侧与商品侧匿名特征 ====================
    print("====== 3.5 合并用户侧与商品侧匿名特征 ======")

    # 统一ID类型，避免 merge 时因为字符串/数字类型不同导致匹配失败
    user_df["uid"] = user_df["uid"].astype(str)
    item_df["iid"] = item_df["iid"].astype(str)

    train_feat["uid"] = train_feat["uid"].astype(str)
    test_feat["uid"] = test_feat["uid"].astype(str)

    train_feat["cand_item"] = train_feat["cand_item"].astype(str)
    test_feat["cand_item"] = test_feat["cand_item"].astype(str)

    # 合并用户侧匿名特征
    train_feat = train_feat.merge(user_df, on="uid", how="left")
    test_feat = test_feat.merge(user_df, on="uid", how="left")

    # 合并商品侧匿名特征
    item_feat = item_df.rename(columns={"iid": "cand_item"})
    train_feat = train_feat.merge(item_feat, on="cand_item", how="left")
    test_feat = test_feat.merge(item_feat, on="cand_item", how="left")

    # 缺失值填充
    train_feat = train_feat.fillna(-1)
    test_feat = test_feat.fillna(-1)
    ...
    # 自动加入 user.csv 和 item.csv 中的匿名数值特征
    extra_feature_cols = [
        col for col in train_feat.columns
        if col not in ['uid', 'cand_item', 'label', 'prediction']
           and col not in base_feature_cols
           and train_feat[col].dtype != 'object'
    ]

    feature_cols = base_feature_cols + extra_feature_cols

    print("最终使用特征数量：", len(feature_cols))
    print("新增匿名特征：", extra_feature_cols)
